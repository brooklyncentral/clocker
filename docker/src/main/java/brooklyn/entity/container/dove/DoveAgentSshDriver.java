...
public class DoveAgentSshDriver extends AbstractSoftwareProcessSshDriver implements DoveAgentDriver {
    ...
    protected Map<String, Integer> getPortMap() {
        return MutableMap.of("weave", getEntity().getConfig(DoveAgent.WEAVE_PORT));
    }
    ...
    @Override
    public void launch() {
        InetAddress address = getEntity().getAttribute(DoveAgent.WEAVE_ADDRESS);
    ...
    public InetAddress attachNetwork(String containerId) {
        ...
        try {
            Cidr cidr = getEntity().getConfig(DoveInfrastructure.WEAVE_CIDR);
            InetAddress address = getEntity().getConfig(DoveAgent.WEAVE_INFRASTRUCTURE).get();
            ((DoveAgent) getEntity()).getDockerHost().execCommand(BashCommands.sudo(String.format("%s attach %s/%d %s",
