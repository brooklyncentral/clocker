...
public interface DoveAgent extends SoftwareProcess {
    ...
    AttributeSensorAndConfigKey<DoveInfrastructure,DoveInfrastructure> WEAVE_INFRASTRUCTURE = ConfigKeys.newSensorAndConfigKey(DoveInfrastructure.class, "weave.infrastructure", "Weave infrastructure entity");
 
    AttributeSensor<InetAddress> WEAVE_ADDRESS = Sensors.newSensor(InetAddress.class, "weave.address", "IP address of Weave service");
    AttributeSensor<DoveAgent> WEAVE_CONTAINER = Sensors.newSensor(DoveAgent.class, "weave.container.entity", "Weave service entity");

    DockerHost getDockerHost();

    MethodEffector<InetAddress> ATTACH_NETWORK = new MethodEffector<InetAddress>(DoveAgent.class, "attachNetwork");
